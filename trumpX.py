...
.twitter-input-tool:hover {
    background-color: #2a2a2a;
}

/* Streamlit textarea 스타일 수정 */
.fixed-input-container .stTextArea {
    flex: 1;
}

.fixed-input-container .stTextArea > div > div > textarea {
...
.fixed-input-container .stTextArea > div > div > textarea::placeholder {
    color: #71767b !important;
    font-size: 18px !important;
}

.fixed-input-container .stTextArea > div {
    border: none !important;
    background-color: transparent !important;
}

/* 트윗하기 버튼 스타일 */
.tweet-button .stButton > button {
...
.tweet-button .stButton > button:hover {
    background-color: #1a8cd8 !important;
}

.tweet-button .stButton > button:disabled {
    background-color: #0f4e78 !important;
...
.clear-header-button .stButton > button {
...
.clear-header-button .stButton > button:hover {
    background-color: #b91c1c !important;
}

/* 숨기기: Streamlit 기본 레이블들 */
.fixed-input-container .stTextArea > label,
.fixed-input-container .stButton > div > div,
.main-header .stButton > div > div {
...
with col1:
    st.markdown('<h1 class="main-title">MYGA TWEETS</h1>', unsafe_allow_html=True)
with col2:
    st.markdown('<div class="clear-header-button">', unsafe_allow_html=True)
    clear_button = st.button("타임라인 초기화", key="clear_header")
...
<div class="fixed-input-container">
    <div class="twitter-input-wrapper">
        <div class="twitter-input-avatar">You</div>
        <div class="twitter-input-content">
''', unsafe_allow_html=True)
...
st.markdown('''
            <div class="twitter-input-actions">
                <div class="twitter-input-tools">
                    <span class="twitter-input-tool">📷</span>
                    <span class="twitter-input-tool">📊</span>
                    <span class="twitter-input-tool">😊</span>
                    <span class="twitter-input-tool">📍</span>
                </div>
...
with col2:
    ...
    clear_button = st.button("🧹 초기화", key="clear_btn")
    st.markdown('</div>', unsafe_allow_html=True)

with col3:
    send_button = st.button("트윗하기", type="primary", disabled=not user_input.strip())
...
# Enter 키로 전송 (JavaScript)
st.markdown("""
<script>
document.addEventListener('keydown', function(event) {
    if (event.key === 'Enter' && !event.shiftKey) {
        const textArea = document.querySelector('.fixed-input-container textarea');
        if (textArea === document.activeElement && textArea.value.trim()) {
            event.preventDefault();
            const button = document.querySelector('.fixed-input-container button[kind="primary"]');
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'Enter' && !event.shiftKey) {
        ...
        if (textArea === document.activeElement && textArea.value.trim()) {
            event.preventDefault();
            const button = document.querySelector('.tweet-button button[kind="primary"]');
            if (button && !button.disabled) {
                button.click();
            }
        }
